 ...
 *                 |    rate, delay     |
 *                 n2------------------n3
 *    1Gb/s, 0ms   |  QueueLimit = 100  |    1Gb/s, 0ms
 * n1--------------|                    |---------------n5
 *
 */
...
void
MonitorThroughput (Ptr<FlowMonitor> flowMonitor, Ptr<Ipv4FlowClassifier> classifier, std::string filename)
{
  ...
  thrFile.open (filename, std::ios::out | std::ios::app);
    ...
    // time, sourceIP, destinationIP, totalTxPkts, totalTxBytes, totalLostPkts, deltaTxBits,
    // deltaLostPkts
...
main (int argc, char *argv[])
{
  ...
  Config::SetDefault("ns3::TcpSocket::SndBufSize", UintegerValue(4194304)); // 4MB
  Config::SetDefault("ns3::TcpSocket::RcvBufSize", UintegerValue(4194304)); // 4MB
  ...
  tchPfifo.AddInternalQueues (handle, 3, "ns3::DropTailQueue", "MaxSize", QueueSizeValue(QueueSize("1000p")));
  ...
  if (queueDiscs.GetN () > 0)
  {
    ...
    if (dualQ)
    {
      dualQ->TraceConnectWithoutContext ("ProbCL", MakeBoundCallback (&TraceQueueProb, &qProbCL));
      ...
      dualQ->TraceConnectWithoutContext ("ClassicSojournTime", MakeBoundCallback (&TraceQueueSojourn, &qSojournClassic));
      dualQ->TraceConnectWithoutContext ("L4sSojournTime", MakeBoundCallback (&TraceQueueSojourn, &qSojournL4S));
  ...
  if (flowMonitor)
  {
    ...
    for (auto i = stats.begin(); i != stats.end(); ++i)
    {
      ...
      outFile << "  Throughput: " << i->second.rxBytes * 8.0 / (client_stop_time - client_start_time) / 1024 / 1024
                  << " Mbps\n";
      outFile << "  Mean delay: " << i->second.delaySum.GetSeconds() / i->second.rxPackets << " s\n";
    }
    outFile.close();
  }
